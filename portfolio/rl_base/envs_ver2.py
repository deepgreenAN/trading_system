...
field_list = ["cash", "unit_number", "mean_cost_price", "price_array"]
StockStateBase = namedtuple("StockStateBase", field_list)

class StockState(StockStateBase):
    """
    状態クラスnamedtupleをベースとする
    """
    def to_numpy(self):
        """
        ndarrayに変更する．最後に利用するのがいい？
        """
        cash_unit_mean_array = np.array([self.cash, self.unit_number, self.mean_cost_price])
        return np.concatenate([cash_unit_mean_array, self.price_array.copy()], axis=0)  # コピーすることに注意
    ...
    def from_numpy(cls, stock_state_array):
        ...
        price_array = stock_state_array[3:]
        
        return cls(cash=cash, unit_number=unit_number, mean_cost_price=mean_cost_price, price_array=price_array)
...
class NormalizeState():
    def __init__(self, cash_const=1, unit_const=1, price_const=1):
    ...
    def __call__(self, state):
        ...
        new_state = state._replace(cash=cash, unit_number=unit_number, mean_cost_price=mean_cost_price, price_array=price_array)  # 新しいオブジェクト 
...
class OneStockEnv(gym.Env):
    ...
    def reset(self, select_datetime=None, select_stock_name=None):
        ...
        self.all_property = cash + now_price * unit_number * self.one_unit_stocks
    ...
    def step(self, action):
        ...
        reward = self.gein_reward_weight * gein_reward + self.price_reward_weight * price_reward +  self.penalty_weight * penalty # 重み
    ...
    def get_stay_penalty(self, cash, action, unit_number):        
        ...
        if any(condition_list):  # 条件のどれかが当てはまる場合
            ...
            prepenalty = - self.initial_cash * 1
            penalty += prepenalty
        return penalty
    ...
    def get_minus_penalty(self, unit_number, cash, now_price):
        ...
        if unit_number < 0:
            ...
            prepenalty = - self.initial_cash * 1 * minus_abs_unit
            penalty += prepenalty
        
        if cash < 0:
            minus_abs_cash = abs(cash)
            minus_abs_cash_as_unit = minus_abs_cash / (self.one_unit_stocks * now_price)
            prepenalty = - self.initial_cash * 1 * minus_abs_cash_as_unit
            penalty += prepenalty
    ...
    def set_weight(self, gein_reward_weight=0, price_reward_weight=1, penalty_weight=1):
