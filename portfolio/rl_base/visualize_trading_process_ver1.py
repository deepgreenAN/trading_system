...
def plot_trading_process_matplotlib(state_list, info_list, env, title=None, save_path=None, is_save=True):
    ...
    all_property_array = np.array(list(map(lambda info: info["all_property"], info_list)))
    penalty_array = np.array(list(map(lambda info: info["penalty"], info_list)))
    adjusted_penalty_array = np.array(list(map(lambda info: info["penalty"]*env.penalty_weight, info_list)))  # 調整済みであることに注意
    ...
    # 価格，ペナルティ
    ax7 = fig.add_subplot(614)
    ax7.plot(x, price_array, label="price", color="C0")
    ax7.set_ylabel("price [yen]")
    ax7.set_title("penalty")

    ax8 = ax7.twinx()
    ax8.plot(x, penalty_array, label="penalty", color="C4")
    ax8.set_ylabel("penalty [yen]")
    ax8.axhline(y=0, color="black", linestyle="dashed")

    # 報酬，取引報酬(調整済み)，価格報酬(調整済み), 報酬, ペナルティ
    ax9 = fig.add_subplot(615)
    ax9.plot(x, adjusted_gein_reward_array, label="adjusted gein reward", color="C3")
    ax9.plot(x, adjusted_price_reward_array, label="adjusted price reward", color="C2")
    ax9.plot(x, adjusted_penalty_array, label="adjusted penalty", color="C4")
    ax9.plot(x, reward_array, label="reward", color="C5")
    ...
    y_min = min(gein_y_min, price_y_min)
    y_max = max(gein_y_max, price_y_max)
    if y_min != y_max:
        ax9.set_ylim(y_min, y_max)
    ax9.set_ylabel("reward [adjusted yen]")
    ax9.set_title("reward")

    # 全財産・現金の推移
    ax10 = fig.add_subplot(616)
    ax10.plot(x, all_property_array, color="C6", label="all property")
    ax10.axhline(y=all_property_array[0], color="black", linestyle="dashed", label="initial all property")
    ax10.set_ylabel("all property [yen]")

    ax11 = ax10.twinx()
    ax11.plot(x, cash_array, color="C7", label="cash")
    ax11.axhline(y=cash_array[0], color="gray", linestyle="dashed", label="initial cash")
    ax11.set_ylabel("cash [yen]")
    ax11.set_title("all property")
...
def plot_trading_process_bokeh(state_list, info_list, env, title=None, save_path=None, is_save=True, is_html=False):
    ...
    # figure 4
    p4 = bokeh.plotting.figure(plot_width=1200,plot_height=300,title="ペナルティ")
    ...
    p4.extra_y_ranges = {"penalty": Range1d(start=y_min,end=y_max)}
    p4.add_layout(LinearAxis(y_range_name="penalty"), 'right')
    p4.line(x=x,y=penalty_array,line_color=bokeh_colors["C4"],line_width=2,legend_label="penalty",y_range_name="penalty")
    p4.line(x=[0,len(x)],y=[0,0],line_color="black",line_width=2,line_dash="dashed",y_range_name="penalty")

    p4.yaxis[0].axis_label = "価格 [円]"
    p4.yaxis[1].axis_label = "ペナルティ(無調整) [円]"

    # figure 5
    p5 = bokeh.plotting.figure(plot_width=1200,plot_height=300,title="全体報酬・調整済み各報酬")
    p5.line(x=x,y=adjusted_gein_reward_array,line_color=bokeh_colors["C3"],line_width=2,legend_label="adjusted gein reward")
    p5.line(x=x,y=adjusted_price_reward_array,line_color=bokeh_colors["C2"],line_width=2,legend_label="adjusted price reward")
    p5.line(x=x,y=reward_array,line_color=bokeh_colors["C5"],line_width=2,legend_label="reward")
    p5.line(x=[0,len(x)],y=[0,0],line_color="black",line_width=2,line_dash="dashed")
    p5.line(x=x,y=adjusted_penalty_array,line_color=bokeh_colors["C4"],line_width=2,legend_label="adjusted penalty")
    ...
    p5.yaxis[0].axis_label = "報酬(調整) [円]"

    # figure 6
    p6 = bokeh.plotting.figure(plot_width=1200,plot_height=300,title="全財産")
    p6.line(x=x,y=all_property_array,line_color=bokeh_colors["C6"],line_width=2,legend_label="all property")
    p6.line(x=[0,len(x)],y=[all_property_array[0],all_property_array[0]],line_color="black",line_width=2,line_dash="dashed",legend_label="initial all property")
    y_min, y_max = make_y_limit(all_property_array)
    p6.y_range = Range1d(start=y_min, end=y_max)

    y_min, y_max = make_y_limit(cash_array)
    p6.extra_y_ranges = {"cash": Range1d(start=y_min,end=y_max)}
    p6.add_layout(LinearAxis(y_range_name="cash"), 'right')
    p6.line(x=x,y=cash_array,line_color=bokeh_colors["C7"],line_width=2,legend_label="cash",y_range_name="cash")
    p6.line(x=[0,len(x)],y=[cash_array[0],cash_array[0]],line_color="gray",line_width=2,line_dash="dashed",y_range_name="cash",legend_label="initial cash")

    p6.yaxis[0].axis_label = "全財産 [円]"
    p6.yaxis[1].axis_label = "現金 [円]"

    if title is not None:
        sup_title = Div(text=title)
        created_figure = bokeh.layouts.column(sup_title,p1,p2,p3,p4,p5,p6)
    else:
        created_figure = bokeh.layouts.column(p1,p2,p3,p4,p5,p6)
